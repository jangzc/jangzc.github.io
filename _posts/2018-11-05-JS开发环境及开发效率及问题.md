...
---
## 问题
1.隐藏暴露的方法和属性，如果不能隐藏，需要解决外部不通过接口修改属性的问题，也许加一个interObj是个方法
2.事件的重构: 
a. 方法统一
b. 考虑d3.dispatch来管理"
3. 每一个graph，通用组件，两者的设计，涉及封装，数据通信等, Clas可以作为一个模板来用
4. Data的管理，是否需要增加Data Manager来统一管理数据
5. API定义
6. 参数太多
7. 交互层可以抽象出来
8.graph的定义：由哪些组成？组成的部分是否可以定义为数据结构。Constructor中可初始化这些组成
